...
//    writeRegister8 (DW1000_FS_CTRL, 0x0B, 0xA6);                //FS_PLLTUNE for channel 5 (trong tài liệu là 0xBE!)

    // sonlh5: Cấu hình tối ưu cho channel 3
    // DW1000_CHAN_CTRL
    // 5 bit (31-27) preamble code
    // 5 bit (26-22) preamble code
    // 2 bit (21-20) rnssfd tnssfd, để 0 (default)
    // 2 bit (19-18) rxprf 01: 16MHZ, 10: 64MHZ
    // 1 bit (17) dwsfd, đề 0 (default)
    // 9 bit (16-8) dự trữ
    // 4 bit (7 - 4) channel
    // 4 bit (3 - 0) channel
    // Ở đây cấu hình kênh 3, 3, preamble code là 5, pxprf là 01
    writeRegister32(DW1000_CHAN_CTRL, 0, 0x29440033);

    writeRegister32(DW1000_TX_POWER, 0, 0x28282828);            //Power for channel 3

    writeRegister8(DW1000_RF_CONF, 0x0B, 0xD8);                 //RF_RXCTRLH for channel 3
    writeRegister32(DW1000_RF_CONF, 0x0C, 0x00086CC0);          //RF_TXCTRL for channel 3

    writeRegister8(DW1000_TX_CAL, 0x0B, 0xC5);                 //TC_PGDELAY for channel 3

    writeRegister32(DW1000_FS_CTRL, 0x07, 0x08401009);         //FS_PLLCFG for channel 3
    writeRegister8(DW1000_FS_CTRL, 0x0B, 0x56);                //FS_PLLTUNE for channel 3
...
void DW1000::ISR() {
    eventQueue.call(Callback<void()>(this, &DW1000::eISR));
}
